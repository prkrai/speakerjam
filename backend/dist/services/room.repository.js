...
exports.roomRepository = {
    async createRoom(hostUserId) {
        ...
        const room = await pool.query('INSERT INTO rooms (room_code, host_user_id) VALUES ($1, $2) RETURNING id, room_code, host_user_id, created_at, updated_at', [roomCode, hostUserId]);
    ...
    async findMembers(roomId) {
        return (await pool.query('SELECT u.id, u.email FROM room_members rm JOIN users u ON u.id = rm.user_id WHERE rm.room_id = $1 ORDER BY rm.joined_at ASC', [roomId])).rows;
    },
